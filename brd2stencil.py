...
def brd_to_svg(boardpath, PIXELS_PER_MM, PAD_SHRINK_MM):
    try:
        ...
        for elem in root.findall('./drawing/board/elements/element'):
            ...
            if(elib in smd_dict):
                if(epkg in smd_dict[elib]):
                    ...
                    ey0 = elem.get('y')
                    erot0 = elem.get('rot')
                    if erot0 == None: erot0 = 0
                    else: erot0 = erot0[1:]
                    erot = float(erot0) * math.pi/180.0
                    ex = float(ex0)
                    ey = float(ey0)
        ...
        svg_out = open(boardpath[:-4] + ".svg", "w")
...
def main():
    ...
    if f == None and F == None:
        ...
        print "\nProcessing BRD file " + f + "..."
        brd_to_svg(f, ppm, shrink)
    else:
        print "\N* File and folder arguments detected. Defaulting to file. Processing BRD file " + f + "..."
        brd_to_svg(f, ppm, shrink)
